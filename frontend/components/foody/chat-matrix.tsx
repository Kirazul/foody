...
const patterns = [
  ...
  [Array.from({ length: 25 }, (_, i) => i), [], [12], [6, 7, 8, 11, 13, 16, 17, 18, 12], [0, 4, 20, 24, 12], Array.from({ length: 25 }, (_, i) => i)],
...
const staticHighlight = [6, 7, 8, 11, 12, 13, 16, 17, 18]

export function ChatMatrix({ size = 5, dotSize = 2, gap = 2, staticPattern = false }: Props) {
  const [activeDots, setActiveDots] = useState<Set<number>>(new Set())
  const totalDots = useMemo(() => size * size, [size])

  useEffect(() => {
    if (staticPattern) {
      setActiveDots(new Set(staticHighlight.filter((i) => i < totalDots)))
      return
    }
    let patternIndex = 0
    let stepIndex = 0
  ...
  return (
    ...
    >
      {Array.from({ length: totalDots }).map((_, index) => (
        <span
          key={index}
          className={`rounded-full bg-current transition-all duration-150 ease-out ${activeDots.has(index) ? "opacity-100 scale-110" : "opacity-15 scale-75"}`}
